...
def test_output_csv_files_creation(tmp_path):
    ...
    assert (
        len(sa_files) > 0
    ), f"Expected to find 'series_sa.csv' inside {local_output}, but found none."
    assert (
        len(ycal_files) > 0
    ), f"Expected to find 'series_ycal.csv' inside {local_output}, but found none."
    assert (
        len(cal_files) > 0
    ), f"Expected to find 'series_cal.csv' inside {local_output}, but found none."
    ...
    assert (
        some_sa_has_size
    ), "The created 'series_sa.csv' files are completely empty (0 bytes). JDemetra+ crunching failed internally without raising OS exception."

    some_ycal_has_size = any(f.stat().st_size > 10 for f in ycal_files)
    assert (
        some_ycal_has_size
    ), "The created 'series_ycal.csv' files are completely empty (0 bytes)."
