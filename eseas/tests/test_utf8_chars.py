...
def test_turkish_characters_in_workspace_name(tmp_path):
    ...
    assert len(sa_files) > 0, "No series_sa.csv generated for Turkish named workspace! 'encoded_name' fallback might be broken."
    assert any(f.stat().st_size > 10 for f in sa_files), "Output files are 0 bytes! Crunching failed silently."
    
    # Check if the final excel file was successfully collected into output folder
    excel_files = list(local_output.rglob("*.xlsx"))
    assert len(excel_files) > 0, "No combined Excel files were collected from the cruncher output!"
