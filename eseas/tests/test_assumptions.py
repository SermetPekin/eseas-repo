...
from eseas.core.demetra_caller import (
    DemetraCallerLinux,
...
def test_executable_quoting(tmp_path):
    ...
    assert linux_cmd.startswith('"') and linux_cmd.endswith(
        '"'
    ), "Linux caller should wrap path in quotes"
...
def test_workspace_creation_with_spaces_and_special_chars(tmp_path):
    ...
    assert "@eseas_wspace" in str(
        c.local_work_space
    ), "@eseas_wspace dir tracking missing"
...
def test_hardcoded_jwsacruncher_direct_call():
    ...
    assert (
        exe_path.exists()
    ), f"Expected executable {exe_path} to exist from test setup."
